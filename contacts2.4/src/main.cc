...
int func1() {
    ...
    cout << format(
        "正在为联系人添加备注信息, 备注信息由两部分组成, 如果你想完成信息添加, "
        "请在第一部分中直接回车\n");
    for (int i = 0;; ++i) {
        string remark_key, remark_val;
        cout << format("请输入备注的标题信息(第一部分): ");
        getline(cin, remark_key);
        if (remark_key.empty()) break;

        cout << format("请输入备注的内容信息(第二部分): ");
    ...
    for (int i = 0; i < size; ++i) {
        ...
        switch (people.other_contact_case()) {
            case contacts::PeopleInfo::kQq:
                cout << format("备用qq联系方式: {}\n", people.qq());
                break;
            case contacts::PeopleInfo::kWechat:
                cout << format("备用微信联系方式: {}\n", people.wechat());
                break;
            default:
                break;
        }
        ...
        if (!people.remark().empty()) {
            auto remark = people.mutable_remark();
            for (auto it = remark->cbegin(); it != remark->cend(); it++) {
                cout << format("备注信息标题: {}, 备注信息内容: {}\n", it->first, it->second);
            }
        }
